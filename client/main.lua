...
local function ProcessRunStuff(ped, limb)
    if limb ~= nil then    
        if IsInjuryCausingLimp() then
            ...
            Citizen.InvokeNative(0xA01B8075D8B92DF4, PlayerId(), false)
        end
    else
...
local function SetBedCam()
    ...
    bedObject = GetClosestObjectOfType(bedOccupyingData.coords.x, bedOccupyingData.coords.y, bedOccupyingData.coords.z, 0.2, bedOccupyingData.model, false, false, false)
    FreezeEntityPosition(bedObject, true)

    SetEntityCoords(player, bedOccupyingData.coords.x, bedOccupyingData.coords.y, bedOccupyingData.coords.z + 0.05)
...
local function CheckWeaponDamage(ped)
    ...
    for k, v in pairs(QBCore.Shared.Weapons) do
        if Citizen.InvokeNative(0xDCF06D0CDFF68424, ped, GetHashKey(k), 0) then
            detected = true
            if not IsInDamageList(k) then
                QBCore.Functions.Notify(Lang:t('info.status').." | "..v.damagereason, "ambulance")
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    Citizen.InvokeNative(0x166E7CF68597D8B5, player, 200) --SetEntityMaxHealth(ped, 750)
    SetEntityHealth(ped, GetEntityMaxHealth(player)) -- Healthbar 100%
    Citizen.InvokeNative(0xF6A7C08DF2E28B28, player, 1, 1, true) -- Staminabar 100%

    Citizen.InvokeNative(0xC6258F41D86676E0, player, 0, 100) -- Healthcore
    Citizen.InvokeNative(0xC6258F41D86676E0, player, 1, 100) -- Staminacore
    Citizen.InvokeNative(0xC6258F41D86676E0, player, 2, 100) -- DeadEYEcore
        ...
        if healthDamaged then
            ...
            if hit and bodypart ~= 'NONE' then
                ...
                if damageDone >= Config.HealthDamage then
                    if weapon then
                        if (bodypart == 'SPINE' or bodypart == 'UPPER_BODY') or weapon == Config.WeaponClasses['NOTHING'] then
