...
Laststand.ReviveInterval = 300 -- time to be to lie on the ground
Laststand.MinimumRevive = 290 -- time to lie on ground until dead
InLaststand = false
LaststandTime = 0
isEscorted = false
    ...
    if bool then
        ...
        CreateThread(function()
            while InLaststand do
                ped = PlayerPedId()
                player = PlayerId()                
                newtime = LaststandTime - 1
                
                if newtime > Laststand.MinimumRevive then
                    LaststandTime = newtime
                    Config.DeathTime = LaststandTime
                elseif newtime <= Laststand.MinimumRevive and newtime ~= 0 then
                    LaststandTime = newtime
                    Config.DeathTime = LaststandTime
